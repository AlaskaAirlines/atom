...
internal extension Retryable {
    ...
    func appliedAuthorizationHeader(_ method: AuthenticationMethod) -> Self {
        var request = self.request
        request.addValue(method.headerValue, forHTTPHeaderField: method.headerField)

        return Retryable(request: request, requiresAuthorization: requiresAuthorization, completion: completion)
    }
}
