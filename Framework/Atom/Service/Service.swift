...
public class Service {
    ...
    private lazy var authenticationManager: AuthenticationManager = {
        AuthenticationManager(serviceConfiguration.authenticationMethod, delegate: self)
    }()
...
internal extension Service {
    ...
    func update(with requestable: Requestable) -> Service {
...
public extension Service {
    ...
    func resume<T>(expecting type: T.Type, completion: @escaping (Result<T, AtomError>) -> Void) where T: Model {
        ...
        do {
            ...
            let retryable = Retryable(request: request, requiresAuthorization: requestable.requiresAuthentication) { data, response, error in
                ...
                // Process error response returned by the service.
                    ...
                    queue.async { completion(Result(AtomError.response(AtomResponse(data, response)))) }
                }
                ...
                // Process data returned by the service.
                    ...
                    if let value = data as? T {
                        ...
                        // Call completion on the queue `Service` was configured to use.
    ...
    ///
    /// - Parameters:
    ///   - completion: The completion containing `Result` where the associated value is either an `AtomError` or `AtomResponse`.
        ...
        do {
            ...
            let retryable = Retryable(request: request, requiresAuthorization: requestable.requiresAuthentication) { data, response, error in
                ...
                // Process response returned by the service.
                    ...
                    queue.async { completion(Result(AtomResponse(data, response))) }
                }

                // Service returned invalid response (based on `statusCode`). Initialize `failure` case with response and optional `data` object.
                else { queue.async {
                    completion(Result(AtomError.response(AtomResponse(data, response)))) }
                }
            }
...
private extension Service {
    func resume(_ retryable: Retryable) {
        let queue = serviceConfiguration.dispatchQueue
