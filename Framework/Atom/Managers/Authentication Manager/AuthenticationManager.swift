...
internal final class AuthenticationManager {
    ...
    internal init(_ method: AuthenticationMethod, delegate: AuthenticationManagerDelegate) {
        self.method = method
...
internal extension AuthenticationManager {
    ...
    func applyAuthorizationHeader(to retryable: Retryable, on queue: DispatchQueue) -> Status {
        switch method {
        case .basic:
    ...
    func refreshAccessToken(_ writable: TokenCredentialWritable, queue: DispatchQueue) {
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            // Process error response returned by the service.
                ...
                queue.async { unownedSelf.delegate.authenticationManagerDidFailToRefreshAccessTokenWithError(.response(AtomResponse(data, response))) }
            }
            ...
            // Process data returned by the service.
                ...
                do {
                    ...
                    writable.tokenCredential = try JSONDecoder().decode(TokenCredential.self, from: data)
